...
\color{red}The Thesis Abstract is written here (and usually kept to just this page). The page is kept centred vertically so can expand into the blank space above the title too\ldots\color{black}

\color{red}TODO: immutable vector with efficient append, prepend, update, getElementAtIndex. optimizations\ldots\color{black}

\color{red}TODO: rrb-trees vector and generailize the same optimizations\ldots\color{black}

\color{red}TODO: result: clear improvement con concatenation. Other operations achieve the same performance when the tree is not too unbalanced, otherwise there is an overhead to be expected, but operations are still nicely bounded\ldots\color{black}
